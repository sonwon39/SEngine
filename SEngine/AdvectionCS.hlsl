...
void main( uint3 DTid : SV_DispatchThreadID )
{
	...
	float2 currPos = float2(DTid.x, DTid.y) + 0.5f;
	float2 currUV = float2(currPos.x / width, currPos.y / height);
	float2 prevPos = currPos - deltaTime * gOldVelocity.SampleLevel(gWarpLinearSampler, currUV, 0).xy;
	float2 prevUV = float2(prevPos.x / width, prevPos.y / height);

	float2 newVelocity = gOldVelocity.SampleLevel(gWarpLinearSampler, prevUV, 0).xy;
	float4 newDensity = gOldDensity.SampleLevel(gWarpLinearSampler, prevUV, 0);
