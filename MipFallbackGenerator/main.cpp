...
void ConvertWICToDDS(const std::wstring& textureFolder, const std::wstring& texconvPath, const std::wstring& outputDir)
{
    for (auto& entry : fs::directory_iterator(textureFolder))
    {
        ...
        std::wstring command =
            texconvPath + L" -f " + format + L" -m 0 -o \"" + outputDir + L"\" \"" + fullPath + L"\"";
...
bool GenerateDDS(const std::wstring& ddsPath, const std::wstring& fileName, const std::wstring& fallbackPath,
                 bool isAlbedo)
{
    ScratchImage image;
    ...
    hr = SaveToDDSFile(fallback.GetImages(), fallback.GetImageCount(), fallback.GetMetadata(), DDS_FLAGS_NONE,
                       fallbackPath.c_str());
    if (FAILED(hr))
    {
